...
project_name='DAPO'
exp_name='Deepseek-R1-1.5B-DAPO-CoT6K-LoRA'
adv_estimator=grpo
kl_coef=0.0
kl_loss_coef=0.0
clip_ratio_low=0.2
...
TRAIN_FILE=${TRAIN_FILE:-"${RAY_DATA_HOME}/data/dapo-math-17k-deepseek-prompt.parquet"}
...
max_packed_length=$((1024 * 9))  # For sequence packing
gen_prompt_bsz=16  # Should be equal to train_prompt_bsz if enable_filter_groups is False, basically we oversample to filter out too easy or too hard prompts
train_prompt_bsz=8  # Real batch size that will be picked for training
train_prompt_mini_bsz=8  # ppo mini batch size, which, in my understanding, does not rely on the filter groups
